...
مصدر الحقيقة: `src/components/app-shell/app-shell.tsx` (المصفوفات `primaryItems` / `advancedItems` / `utilityItems`).
...
3. مكتبتي والإعدادات في نهاية القائمة كأدوات شخصية ثابتة.
4. الروابط تخفي تسمياتها في وضع rail وتظهر كاملة في الوضع الموسّع.
5. لوحة الأوامر ⌘K تبحث في الـ15 وجهة كلها — أي وجهة جديدة تُضاف للمصفوفات الثلاث تلقائيًا تدخل اللوحة.
...
| 24 | `/app/models/[modelId]` | صفحة نموذج | **معطوب — مجلد `odelId]`** | Ops |
| 25 | `/app/models/routing` | توجيه النماذج | فرعي من 23 | Ops |
| 26 | `/app/runs` | سجل التشغيلات | ⚠️ **يتيم** | Ops |
| 27 | `/app/runs/[runId]` | تفاصيل تشغيل | ⚠️ فرعي من يتيم | Ops |
| 28 | `/app/usage` | الاستخدام والتكلفة | ⚠️ **يتيم** | Ops |
| 29 | `/app/billing` | الفوترة | ⚠️ **يتيم** | Ops |
| 30 | `/app/team` | الفريق والأدوار | ⚠️ **يتيم — صفر روابط داخلية** | Ops |
| 31 | `/app/skills` | المهارات | ⚠️ **يتيم** | Ops |
| 32 | `/app/tools` | الأدوات | ⚠️ **يتيم** | Ops |
...
```mermaid
flowchart TB
    subgraph NAV["القائمة الجانبية — 15 وجهة"]
        direction LR
        subgraph G1["الخدمات"]
            HOME["لك"]
            CHAT["اسأل"]
    ...
    subgraph ORPHAN["طبقة التشغيل اليتيمة"]
        direction LR
        RUNS["التشغيلات<br>+ التفاصيل"]
        USAGE["الاستخدام<br>والتكلفة"]
        BILL["الفوترة"]
        TEAM["الفريق<br>والأدوار"]
        CAT["المهارات ·<br>الأدوات"]
    end
...
قراءة الخريطة: الطبقة اليتيمة ليست ميتة — جزء منها مترابط داخليًا (usage ↔ billing، runs تُفتح من المشاريع والتدفقات) — لكن لا مدخل تنقل ثابتًا إليها من الغلاف. هذا يعني أن مستخدمًا لا يعرف الرابط المباشر **لن يجدها أبدًا**، وهو سبب الالتباس الذي ظهر عند مقارنة اللقطات بالموقع.

**القرار المعماري المطلوب (مرجع:** [known-issues.md](../05-process/known-issues.md)**):** دمج طبقة التشغيل في التنقل بمجموعة رابعة «التشغيل» أو إخفاؤها خلف /app/home — التوثيق الحالي يرجّح الدمج لأن الصفحات منجزة ومرتبطة بالوعد المنتجي (الشفافية في التكلفة والفريق).
...
| ask | — (البوابة المشتركة) | prototype عبر `/app/chat` | gateway |
...
3. مجموعات التنقل الثلاث لا تُفكك — يمكن إضافة مجموعة رابعة بقرار موثّق فقط.
4. المسارات الديناميكية `[param]` تسمّى بأسماء صريحة صحيحة — أي مجلد معطوب مثل `odelId]` يُصلح قبل أي عمل آخر على نطاقه.
