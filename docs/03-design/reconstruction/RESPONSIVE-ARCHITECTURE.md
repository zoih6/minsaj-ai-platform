...
| **Tablet** | 768–1023 (fine pointer) | **Expanded labeled sidebar (push)** — DEFAULT since W-6 (owner decision D-2/RES-02: the §7.1 default implemented 2026-09-25 — 264px, labels visible, main pushed; the old rail-default + expand-overlay grammar deleted); rail remains the persisted user-collapsed choice | 2 columns where the container affords (`Grid` auto-fill); `Split` active from ~880 container width; overlays push, not cover, except dialogs. | Ramps unclamp; dock hidden (sidebar owns navigation); header search expands inline. |
