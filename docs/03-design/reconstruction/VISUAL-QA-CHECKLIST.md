...
- [ ] **B-6 Responsive composition** *(amended W-7 / owner decision D-6)* — composition is chosen by WIDTH alone (drawer <768 · expanded sidebar ≥768): desktop-mode-on-phone serves the desktop composition exactly as standard websites do. The coarse-pointer guard and the never-scale zoom are RETIRED (R-RES-1a/1b overridden by D-6); the 44px touch floor continues to apply at true phone widths (≤767.98px). `Split`/`Grid`/`DataList` reflow contracts hold. *(R-RES-2, R-GRD-1.)*
...
- [ ] **B-11 Adversarial perceptual gate** *(added W-7, §7.9 — the owner's post-deployment directive)* — for any wave claiming visual-quality change: full-page captures at 390/768/1440 + desktop-mode-on-phone, reviewed by a **critical comparative VLM pass against the designated reference page (اسأل)**, prompted to find flaws (not to approve). A page passes only when the critique finds no P0/P1 visual defect AND judges the page at the reference's quality level. Consistency metrics alone (G-6/G-7 harmony probes) can NEVER close a visual-quality claim; a rubber-stamp approval ("everything looks good") is a protocol violation and voids the gate. Evidence lands in `evidence/baselines/<wave>/` with the verbatim critique.
...
- [ ] **G-7 Token lint** (stylelint + custom checks): no raw spacing values, no undeclared radii/shadows, no sub-10px text, letter-spacing without Arabic override, no new component classes outside the primitive layer, `globals.css` selector count trends to zero.
  - **Live since Phase 0 (2026-09-22); ERROR MODE since Phase 6 (2026-09-24):** `.stylelintrc.json` + `scripts/token-lint.mjs` (`npm run lint:css` / `npm run lint:tokens`); frozen baseline `evidence/baselines/phase0/g7-baseline.json` (155 raw paddings · 15 undeclared radii · 72 raw shadows · 29 raw font sizes · 23 sub-10px declarations · 37 tracking candidates · 1766 globals selectors · 67 frozen component classes). CI: `.github/workflows/visual-qa-guards.yml` — **the ratchet is enforced on every push**: the stylelint half runs with `--max-warnings 929` (the Phase 6 frozen count) and the custom half runs with `--enforce --compare` the phase5 baseline, exiting red on any absolute-zero regression (sub-10px, unpaired tracking, new component classes), any census metric rising (distinct or occurrences), or any per-file occurrence increase (debt may not migrate between files). Sanity-proven at freeze time: an injected `font-size: 9px; padding: 17px` was caught on all five axes before being reverted.
  - **Phase 1 (2026-09-22) — checks upgraded:** per-selector Arabic-tracking pairing (a `letter-spacing ≠ 0` is legal only under `[lang="en"]` scope, a latin-only class, or an rtl/ar zero-override companion — headline metric `unpairedTracking`, goal 0) + §10 legacy-name var() census (`legacyTokenReferences`, trend to 0 at wave end) + comment-preserving line numbers. Phase 1 close: **sub-10px 23→0 · unpaired tracking 37→0 · raw paddings 155→141 · radii 15→12 · font sizes 29→26** (baseline `evidence/baselines/phase1/`).
- [x] **G-6 Shell probe** (CI, on every push since Phase 6): headless run of the 390/430/980-coarse probes asserting B-5/B-6 metrics, plus 1024/1440 fine-pointer regression controls. **Live since Phase 6 (2026-09-24):** `scripts/visual-qa/shell-probe.mjs` (`npm run qa:shell`, CI job `g6-shell-probe` — build → start → probe, exit 1 on any FAIL). Measures composition (`data-sidebar` + dock visibility), chrome budgets (≤15.5% @390 / ≤14.5% @430 / ≤15.5% @980-coarse), single-row/scaled topbar, horizontal overflow, per-route frozen touch-target caps, the never-scale effective-text floors (body ≥ 14px / mono ≥ 11px / nav ≥ 12px / dock ≥ 10px effective at the 414px-phone scale 0.422), and the drawer-open behavior at 980 coarse. Evidence: `evidence/baselines/phase6/` (before: 3/3 coarse routes FAIL exactly as RES-01 predicted · after: 11/11 PASS).
...
**LIVE since Phase 6 (2026-09-24):** `scripts/visual-qa/signoff.mjs` classifies every push's diff into the four classes above (precedence: token > pattern > route > copy; new route files count as pattern changes), prints the required gates and approver, and `--check` — wired into the `signoff-matrix` CI job — **rejects Owner-class pushes that do not carry a dated entry in `docs/05-process/SIGN-OFF-LOG.md` in the same push** (chat approval first, ledger entry with the code). PRs carry the §5 checklist via `.github/PULL_REQUEST_TEMPLATE.md`.
