...
export function TeamPrototype({ locale, initialMembers }: { locale: Locale; initialMembers: TeamMember[] }) {
  ...
  ]} /><div className="mj-data-list team-data-list ops-data-list" role="table" aria-label={ar ? "أعضاء الفريق" : "Team members"}><div className="mj-data-list__head" role="row"><span>{ar ? "العضو" : "Member"}</span><span>{ar ? "الدور" : "Role"}</span><span>{ar ? "الوصول" : "Access"}</span><span>{ar ? "آخر نشاط" : "Last active"}</span><span /></div>{visible.map((member) => <div className="mj-data-list__row" role="row" key={member.id}><span className="ops-cell-id" data-label={ar ? "العضو" : "Member"}><i>{member.initials}</i><span><strong>{localize(member.name, locale)}</strong><small>{member.email}</small></span>{member.role === "owner" ? <Crown size={14} /> : null}</span><span data-label={ar ? "الدور" : "Role"}><label className="inline-role-select"><select aria-label={ar ? `دور ${localize(member.name, locale)}` : `${localize(member.name, locale)} role`} value={member.role} disabled={member.role === "owner"} onChange={(event) => { const nextRole = event.target.value as TeamMember["role"]; setMembers((items) => items.map((item) => item.id === member.id ? { ...item, role: nextRole } : item)); setNotice(ar ? "تغير الدور محليًا وسُجلت معاينة الأثر." : "Role changed locally and the impact preview was recorded."); }}>{(Object.keys(roleLabels) as TeamMember["role"][]).map((id) => <option key={id} value={id}>{localize(roleLabels[id], locale)}</option>)}</select><ChevronDown size={14} /></label></span><span data-label={ar ? "الوصول" : "Access"}><Badge tone={member.status === "active" ? "success" : member.status === "pending" ? "warning" : "danger"}>{member.status === "active" ? (ar ? "نشط" : "Active") : member.status === "pending" ? (ar ? "دعوة معلقة" : "Pending invite") : (ar ? "موقوف" : "Suspended")}</Badge></span><span data-label={ar ? "آخر نشاط" : "Last active"}>{member.lastActiveAt ? (ar ? "مؤخرًا" : "Recently") : (ar ? "لم ينضم" : "Not joined")}</span><button className="icon-button" type="button" aria-label={ar ? `إجراءات ${localize(member.name, locale)}` : `${localize(member.name, locale)} actions`}><MoreHorizontal size={16} /></button></div>)}</div><section className="roles-explainer"><div className="section-heading"><div><p className="section-kicker">{ar ? "نموذج الصلاحيات" : "Permission model"}</p><h2>{ar ? "الأدوار لا تُخفي السلطة" : "Roles make authority explicit"}</h2></div><button className="button button--outline button--compact" type="button"><KeyRound size={14} />{ar ? "عرض المصفوفة" : "View matrix"}</button></div><div>{(["admin", "builder", "reviewer", "viewer"] as TeamMember["role"][]).map((id) => <article key={id}><span>{id === "admin" ? <ShieldCheck size={16} /> : id === "reviewer" ? <UserCheck size={16} /> : id === "builder" ? <Users size={16} /> : <Check size={16} />}</span><strong>{localize(roleLabels[id], locale)}</strong><p>{roleSummary(id, ar)}</p></article>)}</div></section>{notice ? <DemoToast message={notice} /> : null}</div>;
}
