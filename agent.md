...
## 2) الحالة الراهنة (آخر تحديث: 2026-09-16 — المرحلة 8)

### الملخص التنفيذي
اكتملت 8 مراحل: (1) إعادة تصميم شاملة بهوية موحّدة، (2) توثيق وتغليف v2، (3) هندسة تخطيط استجابي كاملة، (4) إصلاح عيب تغليف v3، (5) إصلاح تداخلات مرئية أبلغ عنها المالك، (6) منظومة وثائق احترافية (10 وثائق) + سبرنت تجربة R1 (حالات UX + حركة + أداء)، (7) تجهيز النشر: GitHub/Vercel، (8) إصلاح تجاوب صفحات المحررات على الهاتف + نظام منع الانحدار (حارس حاويات في CI + مسحة 90 فحصاً).
...
- ✅ **90/90 فحص تجاوب (30 مساراً منها 8 ديناميكية × 375/768/1440) — صفر overflow أفقي** (بعد المرحلة 8)
- ✅ **24/24 صفحة داخل سياق حاوية — حارس معماري في CI** (بعد المرحلة 8)
- ✅ خادم إنتاجي محلي: كل المسارات المفحوصة ترجع 200
- ✅ صفر تحذيرات hydration، صفر أخطاء console

### إصلاح المرحلة 8 (الأهم — نمط خطأ يجب تجنبه للأبد)
**الخطأ**: صفحات `agents/new` و`agents/[id]/edit` و`flows/new` و`flows/[id]/edit` كانت خارج أي سياق container query (جذرها `builder-page`/`flow-editor-page` وليس `ops-page`) — فبقيت على شبكة سطح المكتب 874px داخل شاشة 375px، ولوحة التدفق انسحقت إلى صفر بكسل.
**الإصلاح**: `globals.css` — أصبح الجذر نفسه حاوية: `.builder-page, .flow-editor-page { …; container-type: inline-size; container-name: ops-page; }` — انطلقت كل قواعد التجاوب المصمّمة سابقاً فوراً.
...
10. **كل صفحة جديدة جذرها سياق حاوية** (`ops-page` / `service-space` / `universal-library-page` / `adaptive-home` / `builder-page` / `flow-editor-page`) — وإلا لن تنطبق عليها قواعد container queries إطلاقاً وستُسلَّم بتخطيط سطح المكتب على الهاتف (خطأ المرحلة 8 القاتل). يفرض ذلك آلياً: `node scripts/check-layout-guards.mjs`.
11. **قبل أي تسليم/دفع: البوابة الثلاثية** — (1) الحارس المعماري أخضر (2) مسحة `scripts/verify-sweep-v8.sh` كاملة 90/90 (3) lint نظيف. أي مسار جديد يُضاف فوراً إلى مصفوفة ROUTES في المسحة قبل دفعه.
...
**بوابة منع الانحدار (إلزامية قبل كل تسليم — من المرحلة 8)**:
```bash
node scripts/check-layout-guards.mjs   # حارس معماري: كل صفحة داخل سياق حاوية (يعمل في CI بلا متصفح)
# داخل مساحة العمل فقط (يحتاج خادم dev + agent-browser):
bash scripts/verify-sweep-v8.sh        # 30 مساراً (منها 8 فرعية ديناميكية) × 3 شاشات = 90 فحصاً
bash scripts/verify-editor-interactions.sh  # تفاعلات المحررات على الهاتف
```
...
### CI (GitHub Actions)
`.github/workflows/ci.yml`: عند كل push/PR → `bun install` → **layout guards** → `lint` → `build`. فشل أي خطوة يظهر كعلامة ✗ على الـ commit (ومنها صفحة جديدة بلا سياق حاوية).
...
6. **مُرسِلات الفحص السلبية**: فحص يستهدف صنفاً غير موجود ينجح دائماً بلا قيمة (كان: فحص توست يستهدف `.universal-tab-bar` غير الموجود بدلاً من `.universal-shell-mobile-nav`). عند كتابة أي فحص DOM، أثبت أولاً أن المُرسِل يطابق عنصراً موجوداً فعلاً.
7. **تغطية المسح = جزء من التعريف بالاكتمال**: مسحات v3→v6 فحصت القوائم فقط فسلّمت محررات مكسورة على الهاتف (المرحلة 8). أي صفحة تُضاف تحت مسار ديناميكي ([id]/edit) يجب أن تدخل مصفوفة ROUTES بنفس اليوم.
...
6. **2026-09-16 (المرحلة 7)**: إنشاء مستودع GitHub جديد للمشروع الحالي (بدون مساس القديم `zoih6/nasaq-ai` مطلقاً) + تجهيز Vercel + الروابط + هذا الملف. (اكتمل: v6.1)
7. **2026-09-16 (المرحلة 8 — الحالية)**: «مشكلة في التجاوب في الصفحات الفرعية: أدوات متقدمة ← الوكلاء/التدفقات مضغوطة وملخبطة على الهاتف — خطأ كارثي! كيف أضمن العمل معك في المراحل المقبلة؟» → إصلاح جذر الحاويات + نظام منع انحدار ثلاثي الأضلاع (حارس CI + مسحة 90 فحصاً + إصلاح مُرسِل فحص فارغ). (اكتمل: v8)
